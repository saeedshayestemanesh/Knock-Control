function [M,Madv,Mret]= markovMx(pCurve,pCurve_High,m1,m2,m1_High,m2_High)
...
Madv_High = zeros(numStates);
Mret = zeros(numStates);                                        % Create empty square matrix
Mret_High = zeros(numStates);
m1pColIndexes= [0:imax] + min(m1,imax-[0:imax]);                % Define non-zero elements, (i+m1') indexed from zero
m1pColIndexes_High= [0:imax] + min(m1_High,imax-[0:imax]);
m2pColIndexes= [0:imax] - min(m2,[0:imax]);                     % Define non-zero elements, (i-m2') indexed from zero
m2pColIndexes_High= [0:imax] - min(m2_High,[0:imax]);
Madv([1:numStates] + m1pColIndexes * numStates) = (1-pCurve);   % MatrixIndex = row + (colIndexFromZero)*colLength
Madv_High([1:numStates] + m1pColIndexes_High * numStates) = (1-pCurve_High);
Mret([1:numStates] + m2pColIndexes * numStates) = pCurve-pCurve_High;       % MatrixIndex = row + (colIndexFromZero)*colLength
Mret_High([1:numStates] + m2pColIndexes_High * numStates) = pCurve_High;
M=(Madv + Mret+Mret_High);
